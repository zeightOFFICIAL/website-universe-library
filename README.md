...
The universe page is the main page of the site. Representing cards with stellar system information, it allows jumping to all the represented systems. It uses SwiperJS's library as a backbone to show the deck of cards. The user can select either card from the deck using keys (left and right), using mouse dragging, mouse scrolling, or by clicking on the pagination below. It uses different JS scripts for hovering effects and selection events. A combination of SwiperJS and custom JS scripts represents the deck, card selection in order to effectively help with user's navigation. The page utilizes the DB and scripts to fetch a list of systems and all required fields to represent each card.
